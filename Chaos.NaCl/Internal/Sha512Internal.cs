...
namespace Chaos.NaCl.Internal
{
    internal static class Sha512Internal
    {
        ...
        /*internal static void Finalize(out Array8<UInt64> outputState, ref Array8<UInt64> inputState, ref Array16<UInt64> input, ulong totalLengthInBits)
        {
            Array16<UInt64> tempBlock;
            int currentLengthInBits = (int)( totalLengthInBits & 1023);
            if (currentLengthInBits == 0)
            {
                Core(out outputState, ref inputState, ref input);
                tempBlock = default(Array16<UInt64>);
                tempBlock.x0=1ul<< 63;
            }
            else
            {
                tempBlock = input;
                         ulong mask = 1ul << (currentLengthInBits & 63);
            switch (currentLengthInBits>>64)
            {
                case 0:
                    tempBlock.x0 |= mask;
                    break;
            ...
            if (currentLengthInBits>
            {
                Core(out outputState,
            }
            else
            {
                Core(out outputState,ref inputState
            }
            {
                Core(out outputState, );
                     tempBlock = default(Array16<UInt64>);
                tempBlock.x0 = 1ul << 61;
                tempBlock.x15 = totalLengthInBits;
                Core(out outputState, ref outputState,ref tempBlock);
            }

            var temp = new byte[256];//todo: remove allocation
            Array.Copy(message, message.Length - modLen, temp, 0, modLen);
            temp[modLen] = 0x80;
            if (modLen <= 128 - 16 - 1)
            {
                ByteIntegerConverter.StoreBigEndian64(temp, 128 - 8, ((ulong)message.Length) * 8);
                ByteIntegerConverter.Array16LoadBigEndian64(out block, temp, 0);
                Core(out state, ref state, ref block);
            }
            else
            {
                ByteIntegerConverter.StoreBigEndian64(temp, 256 - 8, ((ulong)message.Length) * 8);
                ByteIntegerConverter.Array16LoadBigEndian64(out block, temp, 0);
                Core(out state, ref state, ref block);
                ByteIntegerConverter.Array16LoadBigEndian64(out block, temp, 128);
                Core(out state, ref state, ref block);
            }
        }*/
        ...
        {
            return (x >> 28) ^ (x << (64 - 28)) ^ (x >> 34) ^ (x << (64 - 34)) ^ (x >> 39) ^ (x << (64 - 39));
        }

        private static ulong Sigma1(ulong x)
        {
            return (x >> 14) ^ (x << (64 - 14)) ^ (x >> 18) ^ (x << (64 - 18)) ^ (x >> 41) ^ (x << (64 - 41));
        }
        ...
        {
            Array16<UInt64> block;
            Array8<UInt64> state;
            Sha512Init(out state);
            var modLen = message.Length % 128;
            for (int i = 0; i < message.Length - modLen; i += 128)
            {
                ByteIntegerConverter.Array16LoadBigEndian64(out block, message, i);
                Core(out state, ref state, ref block);
            }
            var temp = new byte[256];//todo: remove allocation
            Array.Copy(message, message.Length - modLen, temp, 0, modLen);
            ...
            ByteIntegerConverter.StoreBigEndian64(result, 0, state.x0);
            ByteIntegerConverter.StoreBigEndian64(result, 8, state.x1);
            ByteIntegerConverter.StoreBigEndian64(result, 16, state.x2);
            ByteIntegerConverter.StoreBigEndian64(result, 24, state.x3);
            ByteIntegerConverter.StoreBigEndian64(result, 32, state.x4);
            ByteIntegerConverter.StoreBigEndian64(result, 40, state.x5);
            ByteIntegerConverter.StoreBigEndian64(result, 48, state.x6);
            ByteIntegerConverter.StoreBigEndian64(result, 56, state.x7);
            return result;
        }*/

        /*private static void Pbkdf2(byte[] password, byte[] salt, int iterations)
        {
            if(password.Length>128)
                password=Sha512(password);


            out Array8<UInt64> output, ref Array8<UInt64> key1, ref Array8<UInt64> key2, ref Array16<UInt64> input, int iterations
            Array16<UInt64> m = input;
            Array8<UInt64> u;
            Core(out u, ref key1, ref m);
