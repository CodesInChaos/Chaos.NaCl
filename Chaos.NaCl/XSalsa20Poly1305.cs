...
namespace Chaos.NaCl
{
    public class XSalsa20Poly1305
    {
        ...
        public static byte[] Encrypt(byte[] message, byte[] key, byte[] nonce)
        {
            ...
            Encrypt(ciphertext, 0, message, 0, message.Length, key, 0, nonce, 0);
            return ciphertext;
        }

        private static void Encrypt(byte[] output, int outputOffset, byte[] message, int messageOffset, int messageLength, byte[] key, int keyOffset, byte[] nonce, int nonceOffset)
        {
            ...
            // first iteration
            {
                ...
                for (int i = 0; i < count; i++)
                    output[16 + outputOffset + i] = (byte)(message[messageOffset + i] ^ tempBytes[i]);
            }
            ...
            while (blockOffset < messageLength)
            {
                ...
                for (int i = 0; i < count; i++)
                    output[16 + outputOffset + blockOffset + i] = (byte)(message[messageOffset + blockOffset + i] ^ tempBytes[i]);
                blockOffset += 64;
            }

            // compute MAC
            Poly1305Donna.poly1305_auth(output, outputOffset, output, outputOffset + 16, messageLength, ref poly1305Key);
        }
    }
}
